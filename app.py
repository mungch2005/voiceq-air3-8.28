...
with tab_memory:
    ...
    st.caption(
        "발언마다 AI가 자동으로 갱신하는 회의 맥락 메모입니다. 다음 발언을 판단할 때 "
        "이 내용을 그대로 참고합니다. AI가 놓쳤거나 잘못 요약한 부분은 실무자가 직접 "
        "고쳐 쓸 수 있습니다 — 전장상황도를 실무자가 미세 조정하는 것과 같은 원리입니다."
    )
    edited_memory = st.text_area(
        "Context Memory 편집",
        value=st.session_state.context_memory_summary,
        height=160,
        key="memory_editor",
        label_visibility="collapsed",
        placeholder="아직 발언이 없습니다.",
    )
