...
[신규 발언]
화자: 당직사관 (대위, 비행단 본부) · 담당분야: 상황 전파 · 영향력 0.40
...
def format_open_events(operation_log: list[dict], limit: int = 30) -> str:
    ...
    limit은 예전엔 5였는데, 이러면 "15시에 발생한 사태를 15시30분까지 처리하다가
    다른 사태가 끼어들어 먼저 처리, 17시에 15시 사태로 복귀"처럼 그 사이 다른
    사태가 5개 넘게 갱신되면 15시 사태가 통째로 프롬프트에서 빠져 LLM이 그 사태를
    "기억"할 방법이 없어지는 문제가 있었다(사용자가 재차 언급해도 새 사태로 잘못
    분류됨). 해커톤 데모 규모에서 사태가 30개를 넘어갈 일은 거의 없으므로, 토큰
    비용보다 "오래된 사태를 잊지 않는 것"을 우선해 limit을 넉넉하게 올렸다.
    """
