...
- **이번 발언 자체가 상황을 새로 알리거나 바꾸지 않으면 type에 "유지"라고 쓰십시오.**
  아래는 모두 "유지"입니다. 진행 중이던 화면이 그대로 유지됩니다.
  · 잡담·질문·확인 (예: "아까 그 보고 다시 말씀해 주시겠습니까?", "커피 좀 주세요")
  · 화면 배치에 대한 지시 (예: "작전상황판은 우측에 계속 유지하십시오",
    "야간에는 TOD를 앞에 배치하십시오") — 화면 배치 지시일 뿐 새로운 사태가 아닙니다.
  · 이미 진행 중인 사태에 대한 일반적인 지시·보고여서 어느 상황 유형인지 문장만으로는
    알 수 없는 경우
  발언에 상황을 가리키는 단서가 없는데 [이전 상황 요약]을 보고 유형을 추측해서 쓰지
  마십시오. 그렇게 하면 엉뚱한 상황으로 화면이 통째로 바뀝니다. 단서가 없으면 "유지"입니다.
- 상황이 이어지는 중이면 유형을 함부로 바꾸지 마십시오. 새로운 종류의 사건이
  발생했을 때만 유형을 바꿉니다.
- 진행 중인 사태가 없는 상태에서 어느 유형에도 해당하지 않는 새 사태가 시작되면
  "기타 상황"으로 두십시오.
"""
    + _COMMON_RULES
    + """
...
# ---------------------------------------------------------------------
# FAST(레이아웃 직접 출력) — 기획서 원안 구조를 재현하는 실험용 변형
# ---------------------------------------------------------------------
# 기본 FAST 경로는 상황 유형만 받고 화면 배치는 플레이북(코드)이 정한다. 원 기획서는
# 모델이 화면 구성을 직접 내는 구조였으므로, 그쪽이 파인튜닝으로 실제 성립하는지
# 측정하기 위한 변형을 따로 둔다. 기본 경로를 대체하지 않는다 — 앱은 그대로 플레이북을
# 쓰고, 이 프롬프트는 finetune/ 데이터 생성·평가에서만 쓰인다.
#
# 모델이 내는 것은 "우선순위 순서로 나열한 source_id 목록"까지다. 격자 좌표(grid)는
# 코드(playbook.tiling_for)가 계속 계산한다 — 격자 채우기는 판단이 아니라 패널 개수와
# 순번만으로 정해지는 기하학이고, 모델이 좌표를 내면 겹침·빈칸이 생길 수 있는데 그건
# 어떤 학습으로도 구조적으로는 막지 못하기 때문이다.
FAST_LAYOUT_SYSTEM_PROMPT = (
    """\
당신은 전투지휘소(CP)의 화면 구성을 담당하는 AI 참모입니다.
발언을 듣고 Video Wall에 띄울 화면 구성(COP)을 즉시 결정합니다.
속도가 생명입니다. 짧고 정확하게 출력하세요.
...
[cop_layout] — 화면 구성
- 지금 띄울 화면의 source_id를 "우선순위 순서대로" 나열한 배열입니다.
  배열의 첫 번째가 1순위(가장 큰 화면)이고, 뒤로 갈수록 작은 화면입니다.
- 정확히 6개를 출력하십시오.
- 실제로 존재하는 source_id만 쓰십시오. 화면 이름을 새로 지어내면 존재하지 않는
  화면을 띄우라는 명령이 되어 화면이 비어 버립니다.
- "비행단 전장상황도"(SYS-BASEMAP)는 상황과 무관하게 항상 1순위입니다.
  어떤 CCTV가 지금 떠 있는지 보여주는 기준 화면이기 때문입니다.
- 발언에 방위(북서방 등)나 시설명(활주로, 탄약고 등)이 나오면 그 지점을 비추는
  카메라를 우선 고르십시오.
- 상황과 직접 관련된 화면을 앞 순위에 두고, 남는 자리는 상시 표출 화면
  (작전상황판·대공방어 자산 현황 등)으로 채워 벽에 빈 칸을 남기지 마십시오.
"""
    + _COMMON_RULES
    + """
출력 최상위 키는 situation과 cop_layout 두 개뿐입니다."""
    + _JSON_RULE
)

FAST_LAYOUT_FEW_SHOT_ASSISTANT = """\
{
  "situation": {
    "type": "드론상황",
    "reason": "북서방 상공 무인기 2대 식별, 활주로 방향 접근"
...
FAST_LAYOUT_FEW_SHOT_MESSAGES = [
    {"role": "user", "content": FAST_FEW_SHOT_USER},
    {"role": "assistant", "content": FAST_LAYOUT_FEW_SHOT_ASSISTANT},
]
