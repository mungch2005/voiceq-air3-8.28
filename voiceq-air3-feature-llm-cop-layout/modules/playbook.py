...
# 시연 벽면에 한 번에 올리는 패널 수. 8칸에 5개여야 1순위(전장상황도)가 2×2를 받는다
# — 6개면 4+5>8이라 2×2가 안 나와 지도가 납작해진다. 여기 있는 이유는 demo.py가
# streamlit을 물고 있어 시험에서 못 읽기 때문이다. 벽면에 무엇이 남고 무엇이 밀리는지가
# 이 값에 걸려 있으므로 시험이 직접 봐야 한다.
DEMO_MAX_PANELS = 5
