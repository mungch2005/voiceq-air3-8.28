...
def _drone_icon_pil(draw, x: float, y: float, color: str) -> None:
    """`_drone_icon_svg`와 같은 쿼드콥터 실루엣을 PIL 도형으로 그린다.

    render_picker_image는 래스터 이미지라 이모지 폰트가 없는 환경에서는 글자가 깨지므로,
    여기서도 도형(선+원)만으로 그려 폰트 의존을 피한다.
    """
    arm = 9.5
    rotor_r = 3.3
    for dx, dy in ((-arm, -arm), (arm, -arm), (-arm, arm), (arm, arm)):
...
# COP 화면 구성 탭의 SVG 지도는 지금까지 무인기만 벡터 아이콘이고 나머지 프리셋은
# 이모지 글자를 그대로 그려서, 조작 탭(전부 벡터 아이콘)과 모양이 서로 달랐다.
# 두 지도가 같은 상황을 같은 모양으로 보여주도록 나머지 4종도 SVG 버전을 맞춰준다.
def _vehicle_icon_svg(x: float, y: float, color: str) -> str:
    parts = [
        f'<rect x="{x - 8}" y="{y - 1}" width="16" height="6" rx="2" fill="{color}"/>',
        f'<rect x="{x - 4.5}" y="{y - 6}" width="9" height="6" rx="1.5" fill="{color}"/>',
    ]
    for wx in (x - 4.5, x + 4.5):
        parts.append(f'<circle cx="{wx}" cy="{y + 5.2}" r="2.2" fill="{color}"/>')
    return "".join(parts)


def _infiltration_icon_svg(x: float, y: float, color: str) -> str:
    r = 9
    return (
        f'<circle cx="{x}" cy="{y}" r="{r}" fill="none" stroke="{color}" stroke-width="2"/>'
        f'<line x1="{x - r * 0.7}" y1="{y + r * 0.7}" x2="{x + r * 0.7}" y2="{y - r * 0.7}" '
        f'stroke="{color}" stroke-width="2"/>'
    )


def _fire_icon_svg(x: float, y: float, color: str) -> str:
    pts = [
        (x, y - 10), (x + 4, y - 3), (x + 6, y + 3), (x + 3, y + 8),
        (x, y + 5), (x - 3, y + 8), (x - 6, y + 3), (x - 4, y - 3),
    ]
...
def _fog_icon_svg(x: float, y: float, color: str) -> str:
    parts = []
    for w, oy in ((12, -5), (7, 0), (11, 5)):
        parts.append(
            f'<line x1="{x - w / 2}" y1="{y + oy}" x2="{x + w / 2}" y2="{y + oy}" '
...
_SVG_ICON_DRAWERS = {
    DRONE_EMOJI: _drone_icon_svg,
    VEHICLE_EMOJI: _vehicle_icon_svg,
    INFILTRATION_EMOJI: _infiltration_icon_svg,
    FIRE_EMOJI: _fire_icon_svg,
    FOG_EMOJI: _fog_icon_svg,
}
...
def build_map_svg(
    ...
    for idx, marker in enumerate(markers or [], start=1):
        ...
        svg_drawer = _SVG_ICON_DRAWERS.get(marker.get("emoji"))
        if svg_drawer:
            p.append(svg_drawer(mx, my, color))
        else:
            p.append(
